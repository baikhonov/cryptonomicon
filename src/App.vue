    ...
    <div class="container">
      ...
      <template v-if="tickers.length">
        ...
        <div>
          ...
          <p>Фильтр: <input v-model="filter" type="text" /></p>
        </div>
        <hr class="w-full border-t border-gray-600 my-4" />
...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=08370e45f2a5e91bf88e5da8f62d4ff02a9da648d2a18f63e934af4ece155e9e`
        );
